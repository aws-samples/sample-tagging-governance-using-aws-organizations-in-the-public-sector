...
export interface SecurityBlogSampleCodeStackProps extends cdk.StackProps {
  organizationalUnitId?: string;
}

export class SecurityBlogSampleCodeStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: SecurityBlogSampleCodeStackProps) {
    super(scope, id, props);

    // Use the parameterized OU ID with a default fallback
    const organizationalUnitIdParam = new cdk.CfnParameter(this, 'OrganizationalUnitId', {
      type: 'String',
      description: 'The ID of the Organizational Unit to apply the SCP policy',
      default: props?.organizationalUnitId,
      constraintDescription: 'Must be a valid AWS Organizations OU ID',
      allowedPattern: 'ou-[0-9a-z]{4,32}-[a-z0-9]{8,32}'
    });
    const ouId = organizationalUnitIdParam.valueAsString;
    ...
    const tagEnforcementSCP = new organizations.CfnPolicy(this, 'TagEnforcementSCP', {
        ...
        targetIds: [ouId] // Using parameterized OU ID
      });
    ...
    // Add CloudWatch Logs permissions for test function
    testFunction.addToRolePolicy(new iam.PolicyStatement({
    ...
    new cdk.CfnOutput(this, 'OrganizationalUnitIdUsed', {
      value: ouId,
      description: 'Organizational Unit ID used for SCP policy target'
    });
